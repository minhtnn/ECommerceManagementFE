...
const ProductSection = ({ title, products, slug, showCarousel = true }: ProductSectionProps) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="relative">
        ...
        <div
          ref={scrollRef}
          className={showCarousel ? "flex gap-4 overflow-x-auto scrollbar-hide pb-4" : "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4"}
        >
          {products.map((product, index) => (
            <div
              key={product.id}
              className={showCarousel ? "min-w-[250px] max-w-[250px] animate-fade-in" : "animate-fade-in"}
      ...
      <div className="flex justify-center mt-8">
        <Link to={`/collections/${slug}`}>
          <Button variant="outline" className="btn-add-cart px-8">
            Xem sản phẩm <span className="font-bold ml-1">{title.toLowerCase()}</span>
          </Button>
        </Link>
      </div>
    </div>
