import { promotionRuleApi, GetPromotionRulesParams } from "@/apis/promotion-rule.api";
import { TCreatePromotionRule, TUpdatePromotionRule } from "@/schemas/promotion-rule.schema";
...
export const usePromotionRule = () => {
    ...
    const getPromotionRuleById = (id: string) =>
        useSuspenseQuery({
            queryKey: [PROMOTION_RULE_QUERY_KEY, id],
            queryFn: () => promotionRuleApi.getPromotionRuleById(id),
        });
    ...
    const updatePromotionRule = () =>
        useMutation({
            mutationFn: ({ id, data }: { id: string; data: Omit<TUpdatePromotionRule, "id"> }) =>
