...
export const useQueryParams = ({
  ...
  const currentChosenValue =
    searchParams.get("chosenValue") || defaultChosenValue;
  const currentPage = parseInt(
    searchParams.get("page") || `${defaultPage}`,
    10,
  );
  const pageSize = parseInt(
    searchParams.get("size") || `${defaultPageSize}`,
    10,
  );
  const name = searchParams.get("name") || `${defaultName}`;
  const sortBy = searchParams.get("sortBy") || defaultSortBy;
  const isAsc =
    (searchParams.get("isAsc") || defaultIsAsc.toString()) === "true";
  ...
  // --- Pagination setters ---
  const setChosenValue = (value: string) => {
    updateParams({ chosenValue: value });
  };
