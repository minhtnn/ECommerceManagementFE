...
import { EPostStatus, POST_STATUS_COLOR, POST_STATUS_LABEL } from "@/types/enums/post-status.enum";
...
const PostEditPage = () => {
    ...
    useEffect(() => {
        if (!post.imageUrl || isImageChanged) return;
        setImagePreview(post.imageUrl);
        setImageError(false);
    }, [post.imageUrl, isImageChanged]);
                            ...
                            {imagePreview ? (
                                <div className="relative w-full max-w-[300px]">
                                    ...
                                    <div className="mt-4 space-y-2">
                                        ...
                                        ) : (!imageError && <p className="text-xs text-muted-foreground text-center">Ảnh hiện tại</p>)}
                                ...
                                <>
                                    ...
                                    <input ref={fileInputRef} id="image-upload-edit" type="file" accept="image/*"
                                        className="hidden" onChange={handleImageChange} />
                                </>
                            )}
                        </div>
                    ...
                    <div className="bg-background rounded-lg border p-6 space-y-4">
                                            ...
                                            {Object.values(EPostStatus)
                                                ...
                                                .map((v) => (
                                                    <SelectItem key={v} value={String(v)}>
                                                        <Badge className={cn(POST_STATUS_COLOR[v as EPostStatus], "text-sm")}>
                                                            {POST_STATUS_LABEL[v as EPostStatus]}
                                                        </Badge>
                                                    </SelectItem>
                                                ))}
                                        </SelectContent>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="title" render={({ field }) => (
                                <FormItem className="col-span-2">
                                    ...
                                    <FormControl><Input placeholder="Nhập tiêu đề" {...field} disabled={updatePostMutation.isPending} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            <FormField control={form.control} name="slug" render={({ field }) => (
                                <FormItem className="col-span-2">
                                    ...
                                    <FormControl><Input placeholder="vd: bai-dang-moi" {...field} disabled={updatePostMutation.isPending} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                        </div>
                        <FormField control={form.control} name="excerpt" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Textarea placeholder="Nhập tóm tắt" rows={3} {...field} disabled={updatePostMutation.isPending} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        <FormField control={form.control} name="content" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Textarea placeholder="Nhập nội dung" rows={6} {...field} disabled={updatePostMutation.isPending} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        ...
                        <div className="flex justify-between items-center pt-2">
                            <p className="text-sm text-muted-foreground">
                                Cập nhật: {formatDateTimeInShort(post.lastModifiedDate ?? post.createdDate)}
                            </p>
